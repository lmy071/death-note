...
function useRoute() { return inject(routeLocationKey) }
...
function createRouterMatcher(routes) {
  ...
  function resolve(rawPath) {
    for (const m of matchers) {
      ...
      if (match) {
        const params = {}
        m.keys.forEach((key, i) => { params[key.name] = match[i + 1] })
...
async function runGuards(to, from) {
  const guards = [ leaveGuards(from), globalBeforeEach,
    updateGuards(from), beforeEnterGuards(to),
    enterGuards(to), globalBeforeResolve ]
...
app.component('RouterLink', {
  ...
  setup(props, { slots }) {
    ...
    return () => h('a', {
      href: '#' + props.to,
      onClick: (e) => { e.preventDefault(); router.push(props.to) }
    }, slots.default?.())
  }
})
```
