  ...
  <div class="page-full bg-page text-body">
    ...
    <div class="ci-stats">
      ...
      <div class="ci-stat-card">
        ...
        <div class="ci-stat-card__label">当前活跃</div>
      </div>
      <div class="ci-stat-card">
    ...
    <div class="ci-list">
      <div v-if="cacheInstances.length > 0" class="ci-section">
        ...
        <div class="ci-section__hint">数据来源：KeepAlive instance.__v_cache</div>
      </div>
      ...
      <TransitionGroup name="ci-item" tag="div" class="ci-items">
        ...
        >
          <div
            ...
            :class="{ 'ci-item__indicator--active': instance.isActive }"
          />
        ...
        <div v-if="confirmDialog.show" class="ci-dialog-overlay" @click.self="confirmDialog.show = false">
          <div class="ci-dialog">
            ...
            <div class="ci-dialog__actions">
              ...
              <button class="btn btn-hover ci-action-btn--danger" type="button" @click="confirmDelete">确认删除</button>
            </div>
          </div>
        </div>
      </Transition>
...
import { useKeepAliveStore, nameLabelMap, refreshCacheSnapshot } from '../../composables/useKeepAliveStore'
...
onMounted(() => {
  ...
  refreshTimer = setInterval(() => refreshCacheSnapshot(), 3000)
})
...
.ci-header {
  ...
  background: rgba(0,0,0,0.15); flex-shrink: 0; gap: 8px;
}
.ci-header__left { display: flex; align-items: center; gap: 12px; }
.ci-header__right { display: flex; align-items: center; gap: 12px; }

.ci-back {
  font-size: 13px; color: rgba(130,177,255,0.9); text-decoration: none; transition: color 0.15s;
}
...
.ci-refresh-btn {
  ...
  color: rgba(230,232,239,0.7); cursor: pointer; transition: all 0.15s;
  display: grid; place-items: center;
}
.ci-refresh-btn:hover { background: rgba(255,255,255,0.05); color: rgba(230,232,239,0.95); }
...
.ci-section__hint { font-size: 11px; color: rgba(230,232,239,0.35); margin-top: 4px; font-style: italic; }
...
.ci-item__name { font-size: 13px; font-weight: 650; }
.ci-item__comp-name {
...
.ci-item__path { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace; }
